...
source & velocity & FWHM & 6300~\AA{} & 6363~\AA{} & $\dot M$\\ 
       & km s$^{-1}$ & km s$^{-1}$ & \multicolumn{2}{c}{$10^{-15}$ erg s$^{-1}$ cm$^{-2}$} &
$10^{-10}\;M_{\odot}$ yr$^{-1}$\\
\hline
...
continuum emission from the two stars has been subtracted. The color scale is chosen to highlight faint emission features; negative values or brighter fluxes are shown as purple or yellow, respectively. \added{The white lines correspond to the means of each of the stars, which were obtained from a Gaussian fit.} Contours show flux densities in $10^{-14}$~erg~s$^{-1}$~cm$^{-2}$~\AA{}$^{-1}$~arcsec$^{-2}$.
\added{The red lines a peaks of emission components (see text for details).} 
\label{fig:DFTau}}
\end{center}
\end{figure}
...
distinguished. They were plotted by doing the Gaussian fit to each of the rows in the vicinity of the jet emission and plotting the mean of those fits. The uncertainty of the resulting fits lies within the line thickness. An example of the resulting fits is shown in Fig~\ref{fig:Jet_Fit}}. We add the fluxes from all fitted Gaussians further than three pixels from the star center for each outflow component
